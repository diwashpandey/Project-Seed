...
import { updateProfilePhotoFetcher } from "../../../../fetchers/Settings/ProfileSection/updateProfilePhotoFetcher";
import { generatePhotoURL } from "../../../../utilities/apiEndpoints";
import DeleteIcon from "../../Icons/DeleteIcon";
import UploadIcon from "../../Icons/UploadIcon";

function UpdateProfilePhotoForm({ formTitle, description }) {
    ...
    const mutation = useMutation(updateProfilePhotoFetcher, {
        onSuccess: () => {
    ...
    return (
        ...
        >
            <form onSubmit={submitHandler} className="flex justify-around pb-14 pt-4 w-[26rem]">
                <div className="h-40 w-40 rounded-3xl border border-dashed border-text-color relative">
                    <img src={generatePhotoURL(user.data.profile_photo)} alt="" className="h-full w-full object-cover rounded-3xl"/>
                ...
                <label htmlFor="profile-photo" className="cursor-pointer inline-block relative">
                    <div className="h-40 w-40 border border-dashed rounded-3xl center flex-col relative border-text-color bg-[rgba(0,0,0,0)] hover:bg-[rgba(0,0,0,0.25)] transition-all">
                        {image.previewURL ? (
                            <>
